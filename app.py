...
with st.sidebar:
    ...
    if moneda == "Colones (₡/kWh)":
        tarifa_crc = st.number_input("Tarifa (₡/kWh)", min_value=1.0, max_value=500.0, value=59.88, step=1.0,
    ...
    if not comparar:
        ...
        area_s    = st.number_input("Área del techo disponible (m²)", min_value=10.0, max_value=1000.0, value=150.0, step=5.0)
        consumo_s = st.number_input("Consumo eléctrico mensual (kWh)", min_value=10.0, max_value=5000.0, value=280.0, step=10.0)
        ...
        inv_max_s = st.number_input("Inversión máxima (USD)", min_value=100, max_value=100_000, value=3000, step=100) if limitar_inv else None
        ...
        area_a    = st.number_input("Área A (m²)",        min_value=10.0,  max_value=1000.0, value=150.0, step=5.0,  key="aa")
        consumo_a = st.number_input("Consumo A (kWh/mes)", min_value=10.0, max_value=5000.0, value=280.0, step=10.0, key="ca")
        cb_a      = st.checkbox("Limitar presupuesto A", key="cba")
        inv_a     = st.number_input("Presupuesto máx. A (USD)", min_value=100, max_value=100_000, value=3000, step=100, key="ia") if cb_a else None

        st.markdown('<div class="sb-scen-b">🌿 Escenario B</div>', unsafe_allow_html=True)
        area_b    = st.number_input("Área B (m²)",        min_value=10.0,  max_value=1000.0, value=200.0, step=5.0,  key="ab")
        consumo_b = st.number_input("Consumo B (kWh/mes)", min_value=10.0, max_value=5000.0, value=400.0, step=10.0, key="cb")
        cb_b      = st.checkbox("Limitar presupuesto B", key="cbb")
        inv_b     = st.number_input("Presupuesto máx. B (USD)", min_value=100, max_value=100_000, value=4000, step=100, key="ib") if cb_b else None

        st.markdown('<div class="sb-scen-c">☀️ Escenario C</div>', unsafe_allow_html=True)
        area_c    = st.number_input("Área C (m²)",        min_value=10.0,  max_value=1000.0, value=100.0, step=5.0,  key="ac")
        consumo_c = st.number_input("Consumo C (kWh/mes)", min_value=10.0, max_value=5000.0, value=180.0, step=10.0, key="cc")
        cb_c      = st.checkbox("Limitar presupuesto C", key="cbc")
        inv_c     = st.number_input("Presupuesto máx. C (USD)", min_value=100, max_value=100_000, value=2000, step=100, key="ic") if cb_c else None
...
with tab2:
    if not run:
        ...
        if comparar:
            st.markdown('<div style="font-size:0.78rem;color:#475569;margin-bottom:1rem">Mostrando análisis financiero del primer escenario con solución óptima. Consulta la tabla comparativa en la pestaña anterior para ver todos los escenarios.</div>', unsafe_allow_html=True)
